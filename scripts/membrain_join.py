...
OUT_MERGED = Path("out/salescogs/merged_forecast_rates.csv")
OUT_LOG = Path("out/logs/run_summary.json")
...
def load_and_group_rates(path: Path) -> pd.DataFrame:
    df = pd.read_csv(
        ...
        usecols=["product", "plant", "element_code", "period_text", "rate", "qty"],
    )
    _, period_key = parse_period_to_key(df["period_text"])
    df["PeriodKey"] = period_key
    grouped = (
        df.groupby(["product", "plant", "element_code", "PeriodKey"], dropna=False)
        .agg(rate=("rate", "sum"), qty=("qty", "sum"))
        .reset_index()
    )
...
def apply_carry_forward(df: pd.DataFrame) -> Tuple[pd.DataFrame, int, int]:
    ...
    df = df.sort_values(["Plant", "PlantProductMesh", "element_code", "Period_dt"])

    df["rate_fill_flag"] = "original"

    plant_period_group = ["Plant", "PlantProductMesh", "PeriodKey"]
    plant_element_group = ["Plant", "PlantProductMesh", "element_code"]

    all_zero_forward = df.groupby(plant_period_group)["Rate"].transform(lambda s: (s == 0).all())
    forward_fill = (
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Join Membrain forecasts with plant rates.")
